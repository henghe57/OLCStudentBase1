...
shortest_distance = 1000000
predicted_weather_type = ""

for weather in weather_records:
    weathers = weather_records[weather]
    # print(weathers)

    existing_temp = weathers[0]
    existing_humidity = weathers[1]
    existing_ws = weathers[2]
    weather_type = weathers[3]

    distance = math.sqrt((temp - existing_temp) ** 2 + (humidity - existing_humidity) ** 2 + (wind_speed - existing_ws) ** 2)

    if distance < shortest_distance:
        shortest_distance = distance
        predicted_weather_type = weather_type 

print(f"Predicted weather type: {predicted_weather_type}")
